...
def try_find_span(G, formula_path_prefix, timeout=120, detailed_results_file_path=None):
    '''Attempts to find a lambda_num for a graph with the given number of nodes. Returns True if a lambda_num is found, False otherwise.'''
    # generate the graph
    ...
    while True:
        print(f"Trying to find a lambda_num for graph with {number_of_nodes} nodes and k={k}...")
        ...
        if satisfiable is False:
            ...
            k -= 1
            if k < 0:
                raise Exception(f"Unexpected result: k became negative while trying to find a lambda_num for graph.")
        

def main():
    errors = []
    result_file_path = "experiments/find_span/results.csv"
    with open(result_file_path, "w") as f:
        f.write("Number of nodes, lambda_num, Time taken (seconds), Timeout happened\n")
        f.close()
    ...
    for n in [5, 10, 15, 20, 30, 40, 50, 60, 70, 80, 100, 120, 150, 200, 250]:
        ...
        print(f"Trying to find a lambda_num for Barabási–Albert graph with {n} nodes...")
        try:
            lambda_num, did_timeout_happen, elapsed_time = try_find_span(G, formula_path_prefix, timeout=120, detailed_results_file_path=detailed_results_file_path)
            print(f"Found lambda_num: {lambda_num}, Time taken: {elapsed_time:.4f} seconds, Timeout happened: {did_timeout_happen}")
            with open(result_file_path, "a") as f:
                f.write(f"{n}, {lambda_num}, {elapsed_time:.4f}, {did_timeout_happen}\n")
                f.close()
        except Exception as e:
            print(f"Error occurred while trying to find a lambda_num for Barabási–Albert graph with {n} nodes, Error: {e}")
